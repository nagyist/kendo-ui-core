...
description: "Explore the Kendo UI for jQuery DatePicker with disabled and selected dates, navigation depth, templates, DateInput integration, and globalization."
...
It provides options for using custom templates for its **Month** view, setting minimum and maximum dates, configuring the start view and navigation depth, and integrating the Kendo UI for jQuery DateInput.
...
| [Input value validation]({% slug validation_datepicker %}) | While, by design, the DatePicker keeps its input value unchanged, it still provides options for you to use a client-validation framework and render prompt error messages. |
...
| [Templates]({% slug templates_datepicker %}) | The DatePicker supports templates that let you customize elements such as cells, footer content, and the week column. |

...
let CLICK = kendo.support.touch ? "touchend" : "click";
...
describe("kendo.ui.ComboBox selection", function() {
    ...
    asyncTest("pressing Capse lock should not open popup", function(done) {
        let calls = 0;
        create({
            filter: "contains",
            delay: 0
    ...
    it("value should be set to item.text if no item.value", function() {
        let localData = [{ text: "Foo", value: 1 }, { text: "Bar" }];
    ...
    it("press Tab should call _blur only when open", function() {
        ...
        let comboboxSpy = spy(combobox, "_blur");
    ...
    asyncTest("empty input should clear focused item", function(done) {
        combobox = new ComboBox(input, {
            dataTextField: "text",
            dataValueField: "value",
            dataSource: data,
        ...
        setTimeout(function() {
            done(() => assert.equal(combobox.current(), null));
        });
    });
    ...
    it("Empty combobox does not select first focused item on ENTER", async function() {
        combobox = new ComboBox(input, {
    ...
    asyncTest("clearing custom value does not re-enter the old value", function(done) {
        create({
            delay: 0,
            filter: "startswith"
        });
        ...
        setTimeout(function() {
            done(() => assert.equal(combobox.input.val(), "", "input is not cleared"));
        });
    });

    asyncTest("clearing custom value does not re-enter the old value (SELECT)", function(done) {
    ...
    it("ComboBox does not focus input if mobile device", function() {
        let origin = kendo.support.touch;
        let input = combobox.input;

...
import { stub } from '../../helpers/unit/stub.js';
    ...
    describe("kendo.ui.ComboBox selection", function() {
        ...
        afterEach(function() {
            let combo = input.data("kendoComboBox");
        ...
        it("value method does not add item with custom value 'null' (select)", function() {
            ...
            let option = select.children(":last");
        ...
        asyncTest("text method does not throw expection if current selected item's value is null", function(done) {
            let combobox = new ComboBox(input, {
                dataTextField: "text",
                dataValueField: "value",
        ...
        it('after enable(false) should not open popup', function() {
            ...
            let oldOpen = combobox.popup.toggle,
                called = false;
        ...
        it("readonly() makes  input element readonly", function() {
            ...
            assert.include(["readonly", "true"], combobox.element.attr("readonly"));
        ...
        it("enable(false) removes readonly attribute and default class", function() {
            ...
            assert.equal(combobox.element.attr("readonly"), undefined);
            assert.include(["disabled", "true"], combobox.element.attr("disabled"));
            assert.equal(combobox.input.attr("readonly"), undefined);
            assert.include(["disabled", "true"], combobox.input.attr("disabled"));
            assert.isOk(combobox.wrapper.hasClass("k-disabled"));
        });
        ...
        it("value(value) calls dataSource.fetch when element is disabled", function() {
            let combobox = new ComboBox(input.attr("disabled", true), {
                autoBind: false
            });
        ...
        asyncTest("ComboBox filter after value method is used", function(done) {
            let combobox = new ComboBox(input, {
                dataSource: ["Item1", "Item2"],
                filter: "contains",
                delay: 0
        ...
        asyncTest("value method selects item that exists only in unfiltered source (async)", function(done) {
            let combobox = new ComboBox(input, {
                dataTextField: "text",
                dataValueField: "value",
                filter: "startswith",
        ...
        it("value method keeps datasource filters if widget filtration is not enabled", function() {
            ...
            let filter = combobox.dataSource.filter();
        ...
        asyncTest("reset filters when _clear is clicked", function(done) {
            ...
            combobox.dataSource.bind("change", function() {
                done(() => assert.equal(JSON.stringify(combobox.dataSource.filter()), JSON.stringify({ "filters": [], "logic": "and" })));
            });
            combobox._clear.click();
        });
    });
